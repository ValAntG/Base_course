class Station
  attr_reader :station_name, :station_train
  ...
  def station_list_train
    ...
    @station_train.each{|train| p train.train_name}
  end

  def station_list_by_type
    p "Список грузовых поездов на станции #{station_name}: "
    @station_train.each{|i| p i.train_name if i.train_type == "cargo"}
    p "Всего грузовых поездов на станции : #{@station_train.select{|i| i.train_type == "cargo"}.size}"
    p "Список пассажирских поездов на станции #{station_name}: "
    @station_train.each{|i| p i.train_name if i.train_type == "pass"}
    p "Всего пассажирских поездов на станции : #{@station_train.select{|i| i.train_type == "pass"}.size}"
  end
